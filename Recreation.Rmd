...
subset_trt <- full_dataset %>% group_by(Population, Treatment) %>% summarize(
  ...
  gluc_se = sd(`Total glucosinolates (µmol/g DW)`, na.rm = TRUE)/sqrt(length(na.omit(`Total glucosinolates (µmol/g DW)`))),
  ...
  bolting_se = sd(`Bolting time (days)`, na.rm = TRUE)/sqrt(length(na.omit(`Bolting time (days)`))),
...
gluc <- ggplot(data=subset_trt)+   
  ...
  geom_point()+
  geom_line()+
  geom_errorbar(aes(y=Glucosinolates, ymin=Glucosinolates-gluc_se, ymax=Glucosinolates+gluc_se), width = 0.05)+
...
TPI <- ggplot(data=subset_trt)+   
  ...
  geom_point()+
  geom_line()+
  geom_errorbar(aes(y=Trypsin_PI, ymin=Trypsin_PI-trypsin_se, ymax=Trypsin_PI+trypsin_se), width = 0.05)+
  scale_shape_manual(values=c(17,15))+
  scale_color_manual(values=c("blue","red"))+
  scale_fill_manual(values=c("blue","red"))+
...
gluc <- ggplot(data=subset_trt)+   
  ...
  geom_point()+
  geom_line()+
  geom_errorbar(aes(y=Trichome_density, ymin=Trichome_density-trichome_se, ymax=Trichome_density+trichome_se), width = 0.05)+
  scale_shape_manual(values=c(17,15))+
  scale_color_manual(values=c("blue","red"))+
  scale_fill_manual(values=c("blue","red"))+
